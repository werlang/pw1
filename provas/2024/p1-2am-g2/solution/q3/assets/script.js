...
const problemsDOM = document.querySelector('#problems');

function build() {
    problems.forEach((problem) => {
        const problemDiv = document.createElement('div');
        problemDiv.classList.add('problem');
        problemDiv.innerHTML = problem.title;

        problemDiv.addEventListener('click', () => {
            showProblem(problem);
        });

        problemsDOM.append(problemDiv);
    });
}
...
function showProblem(problem) {
    // Implemente a função showProblem aqui
    ...
    fog.innerHTML = `<div class="modal">
        ...
        <div>Entrada:</div>
        <div><div class="square">${problem.input}</div></div>
        <div>Saída:</div>
        <div><div class="square">${problem.output}</div></div>
        <div id="button-container"><button>OK</button></div>
    </div>`;

    const button = fog.querySelector('button');
...
// const testProblem = {
//     title: 'Meu Problema!!!!',
//     description: 'Teste de descrição do problema',
//     input: 'Oi Oi Oi',
//     output: '123123'
// }
// showProblem(testProblem);